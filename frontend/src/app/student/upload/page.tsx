      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        {!result ? (
          <>
            ...
            <button onClick={handleSubmit} disabled={!file && !manualInput.trim() || loading} className="btn-primary w-full text-lg py-3">{loading ? "AI 正在批改中..." : "🤖 开始批改"}</button>
          </>
        ) : (
          <div className="space-y-6">
            <div className="card"><h3 className="font-semibold text-lg mb-4">批改结果</h3><div className="text-4xl font-bold text-center text-[#6366f1] mb-4">{result.score}分</div>{result.comments && <div className="p-4 bg-[#eef2ff] rounded-lg mb-4"><p className="text-indigo-800">{result.comments}</p></div>}</div>
